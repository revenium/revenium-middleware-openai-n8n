...
// Extended ChatOpenAI class with Revenium tracking
class ReveniumTrackedChatOpenAI extends ChatOpenAI {
	private reveniumCredentials: ReveniumOpenAICredentials;

	constructor(config: Record<string, unknown>, reveniumCredentials: ReveniumOpenAICredentials) {
		super(config);
	 ...
	 */
	private logResultStructure(result: ChatResult, generation: unknown, message: unknown, responseMetadata: Record<string, unknown> | undefined): void {
		logger.debug('Revenium tracking - full result structure: %O', {
			...
			responseMetadataKeys: responseMetadata ? Object.keys(responseMetadata) : [],
	 ...
	 */
	private findUsageData(responseMetadata: Record<string, unknown> | undefined, message: unknown, generation: unknown): unknown {
		const usageFromResponseMetadata = responseMetadata?.usage;
		const usageFromMessage = hasUsageMetadata(message) ? (message as { usage_metadata: unknown }).usage_metadata : undefined;
		const usageFromGeneration = hasUsageMetadata(generation) ? (generation as { usage_metadata: unknown }).usage_metadata : undefined;
		...
		logger.debug('Usage data locations: %O', {
			...
			usageFromResponseMetadataStructure: usageFromResponseMetadata ? Object.keys(usageFromResponseMetadata) : null,
			usageFromMessageStructure: usageFromMessage ? Object.keys(usageFromMessage) : null,
			usageFromGenerationStructure: usageFromGeneration ? Object.keys(usageFromGeneration) : null
		});
	...
	private async trackUsageWithErrorHandling(
		...
		try {
			await this.trackUsageWithRevenium(messages, result, responseMetadata, usageMetadata as Record<string, unknown> | undefined, duration);
		} catch (error: unknown) {
	...
	async _generate(
		...
		try {
			const { generation, message, responseMetadata } = this.extractGenerationData(result);

			logger.info('OpenAI API call successful, extracting metadata for Revenium...');
			...
			if (responseMetadata) {
				...
				const usageMetadata = this.findUsageData(responseMetadata, message, generation);

				// Fire-and-forget Revenium tracking with comprehensive metadata
				this.trackUsageWithErrorHandling(messages, result, responseMetadata, usageMetadata, duration);
			} else {
				logger.warning('No response metadata found for Revenium tracking');
			}
		} catch (error: unknown) {
			const errorDetails = getErrorDetails(error);
			logger.warning('Error extracting metadata for Revenium tracking: %s', errorDetails.message);
		}
	 ...
	 */
	private setupStreamTimeout(options: ChatOpenAICallOptions): { abortController: AbortController; timeoutId: NodeJS.Timeout; streamTimeout: number } {
	...
	private isFirstTokenChunk(chunk: unknown): boolean {
		return !!(chunk && typeof chunk === 'object' && 'text' in chunk &&
	 ...
	 */
	private extractResponseMetadata(chunk: unknown): Record<string, unknown> | null {
		if (chunk && typeof chunk === 'object' && 'generationInfo' in chunk &&
			chunk.generationInfo && typeof chunk.generationInfo === 'object' &&
	...
	private processStreamingChunk(
		...
		if (!updatedFirstTokenTime && this.isFirstTokenChunk(chunk)) {
			...
			logger.debug('First token received in streaming at: %d ms', updatedFirstTokenTime - startTime);
		}
	...
	private async trackStreamingUsage(
		...
		const endTime = Date.now();
		const duration = endTime - startTime;
		const timeToFirstToken = firstTokenTime ? (firstTokenTime - startTime) : duration;

		logger.debug('Streaming Revenium tracking - final metadata extraction: hasResponseMetadata=%s, hasUsageMetadata=%s, duration=%d, timeToFirstToken=%d, chunkCount=%d',
			!!lastResponseMetadata, !!accumulatedUsage, duration, timeToFirstToken, chunkCount);
		...
		try {
			...
			const errorDetails = getErrorDetails(error);
			logger.warning('Streaming Revenium tracking failed (non-blocking): %s', errorDetails.message);
			// Log additional context in development
			if (process.env.NODE_ENV === 'development') {
				logger.debug('Streaming tracking error context: chunkCount=%d, duration=%d, hasMetadata=%s', chunkCount, duration, !!lastResponseMetadata);
			}
		}
	}
	...
	async *_streamResponseChunks(
		...
		logger.info('Revenium Chat Model - intercepting streaming _streamResponseChunks call');
		...
		const { abortController, timeoutId, streamTimeout } = this.setupStreamTimeout(options);
		const { signal } = abortController;

		try {
			// Call the original streaming method with abort signal
			const streamGenerator = super._streamResponseChunks(messages, options, runManager);
			...
			for await (const chunk of streamGenerator) {
				...
				if (signal.aborted) {
					throw createReveniumError('Stream aborted', undefined, 'STREAM_ABORTED');
				}
				...
				if (currentTime - lastChunkTime > streamTimeout) {
					abortController.abort();
					throw createReveniumError('Stream timeout exceeded', undefined, 'STREAM_TIMEOUT');
				}
				...
				const { updatedFirstTokenTime, updatedResponseMetadata, updatedUsage } =
					this.processStreamingChunk(chunk, startTime, firstTokenTime, lastResponseMetadata, accumulatedUsage);
			...
			const errorDetails = getErrorDetails(error);
			logger.error('Revenium Chat Model - streaming error: %s', errorDetails.message);
			// Re-throw with proper error context
			if (error instanceof Error && error.name === 'ReveniumError') {
				throw error;
			}
			throw createReveniumError(`Streaming failed: ${errorDetails.message}`, error, 'STREAMING_ERROR');
		} finally {
			// Cleanup resources
	 ...
	 */
	private getFieldValue(camelCase: string | undefined, snakeCase: string | undefined): string | undefined {
	...
	private extractSubscriberFields(usageMetadata: UsageMetadata): {
		...
		return {
			id: this.getFieldValue(usageMetadata.subscriberId, usageMetadata.subscriber_id),
			email: this.getFieldValue(usageMetadata.subscriberEmail, usageMetadata.subscriber_email),
			credentialName: this.getFieldValue(usageMetadata.subscriberCredentialName, usageMetadata.subscriber_credential_name),
			credentialValue: this.getFieldValue(usageMetadata.subscriberCredential, usageMetadata.subscriber_credential)
		};
	}
	 ...
	 */
	private hasSubscriberData(id: string | undefined, email: string | undefined, credentialName: string | undefined, credentialValue: string | undefined): boolean {
	 ...
	 */
	private buildCredentialObject(credentialName: string | undefined, credentialValue: string | undefined): { name: string; value: string } | undefined {
	...
	private buildSubscriberInfo(
		...
		const credential = this.buildCredentialObject(credentialName, credentialValue);
		if (credential) {
	 ...
	 */
	private buildSubscriberObject(usageMetadata?: UsageMetadata): SubscriberInfo | undefined {
		if (!usageMetadata) {
			return undefined;
		}

		const { id, email, credentialName, credentialValue } = this.extractSubscriberFields(usageMetadata);
	...
	): { usage: unknown; requestId: string; modelName: string; finishReason: string } | null {
		const generation = result.generations?.[0];
		const message = generation?.message;
	 ...
	 */
	private extractBasicTokenCounts(usage: unknown): { inputTokenCount: number; outputTokenCount: number; totalTokenCount: number } {
		if (!hasTokenUsage(usage)) {
			return { inputTokenCount: 0, outputTokenCount: 0, totalTokenCount: 0 };
		}

		const inputTokenCount = usage.prompt_tokens || usage.promptTokens || usage.input_tokens || 0;
		const outputTokenCount = usage.completion_tokens || usage.completionTokens || usage.output_tokens || 0;
		const totalTokenCount = usage.total_tokens || usage.totalTokens || (inputTokenCount + outputTokenCount) || 0;
	 ...
	 */
	private extractAdvancedTokenCounts(usage: unknown): { reasoningTokenCount: number; cacheReadTokenCount: number } {
		if (!hasTokenUsage(usage)) {
			return { reasoningTokenCount: 0, cacheReadTokenCount: 0 };
		}

		const reasoningTokenCount = usage.completion_tokens_details?.reasoning_tokens || usage.output_token_details?.reasoning || 0;
		const cacheReadTokenCount = usage.prompt_tokens_details?.cached_tokens || usage.input_token_details?.cache_read || 0;
	 ...
	 */
	private extractAudioTokenCounts(usage: unknown): { audioInputTokens: number; audioOutputTokens: number } {
		if (!hasTokenUsage(usage)) {
			return { audioInputTokens: 0, audioOutputTokens: 0 };
		}

		const audioInputTokens = usage.prompt_tokens_details?.audio_tokens || usage.input_token_details?.audio || 0;
		const audioOutputTokens = usage.completion_tokens_details?.audio_tokens || usage.output_token_details?.audio || 0;
	 ...
	 */
	private extractPredictionTokenCounts(usage: unknown): { acceptedPredictionTokens: number; rejectedPredictionTokens: number } {
		if (!hasTokenUsage(usage)) {
			return { acceptedPredictionTokens: 0, rejectedPredictionTokens: 0 };
		}

		const acceptedPredictionTokens = usage.completion_tokens_details?.accepted_prediction_tokens || 0;
		const rejectedPredictionTokens = usage.completion_tokens_details?.rejected_prediction_tokens || 0;
	...
	private extractTokenCounts(usage: unknown): {
		...
		logger.debug('Extracted token counts: input=%d, output=%d, total=%d, reasoning=%d, cached=%d',
			basicCounts.inputTokenCount, basicCounts.outputTokenCount, basicCounts.totalTokenCount,
			advancedCounts.reasoningTokenCount, advancedCounts.cacheReadTokenCount);
	...
	private buildUserMetadata(): Record<string, unknown> {
		...
		if (this.reveniumCredentials.usageMetadata?.organizationId) {
			metadata.organizationId = this.reveniumCredentials.usageMetadata.organizationId;
		}
		if (this.reveniumCredentials.usageMetadata?.subscriptionId) {
			metadata.subscriptionId = this.reveniumCredentials.usageMetadata.subscriptionId;
		}
		...
		if (this.reveniumCredentials.usageMetadata?.responseQualityScore) {
			metadata.responseQualityScore = this.reveniumCredentials.usageMetadata.responseQualityScore;
		}
	 ...
	 */
	private async sendToReveniumAPI(payload: Record<string, unknown>): Promise<void> {
		const reveniumUrl = buildReveniumUrl(this.reveniumCredentials.reveniumBaseUrl, '/ai/completions');
		logger.debug('Revenium API call details: url=%s, apiKeyPrefix=%s, baseUrl=%s',
			reveniumUrl,
			this.reveniumCredentials.reveniumApiKey ? this.reveniumCredentials.reveniumApiKey.substring(0, 8) + '...' : 'MISSING',
		...
		if (!response.ok) {
			...
			logger.error('Revenium API error: status=%d, statusText=%s, body=%s',
		...
		logger.info('Revenium tracking successful: requestId=%s, tokens=%d, response=%s',
			payload.transactionId, payload.totalTokenCount, responseBody);
	}
	...
	private async trackUsageWithRevenium(
		...
		try {
			...
			const trackingData = this.extractTrackingData(result, responseMetadata, usageMetadata);
			if (!trackingData) {
			...
			const subscriber = this.buildSubscriberObject(this.reveniumCredentials.usageMetadata);
			if (subscriber) {
			...
			logger.debug('Sending Revenium tracking payload: requestId=%s, model=%s, tokens=%d, duration=%d, stopReason=%s, isStreamed=%s',
				requestId, modelName, reveniumPayload.totalTokenCount, reveniumPayload.requestDuration, reveniumPayload.stopReason, reveniumPayload.isStreamed);
...
export class ReveniumOpenAIChatModel implements INodeType {
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
				...
				type: 'options',
				noDataExpression: true,
				description: 'The model which will generate the completion. Models are loaded dynamically from OpenAI.',
				typeOptions: {
			...
			},
			{
				...
				default: {},
				description: 'Optional metadata for enhanced Revenium tracking and analytics',
				options: [
					{
				...
				try {
					...
					const sortedModels = chatModels.sort((a: unknown, b: unknown) => {
						...
						const getPriority = (id: string) => {
							...
							if (id.includes('gpt-4o') && (id.includes('2024-11-20') || id.includes('2024-08-06'))) return 880;
							...
							if (id.includes('gpt-4') && !id.includes('gpt-4o') && !id.includes('gpt-4.1')) return 680;
							...
							if (id.includes('audio') || id.includes('realtime') || id.includes('search')) return 400;
					...
					const modelOptions: INodePropertyOptions[] = sortedModels.map((model: unknown) => {
						...
						else if (modelId.startsWith('gpt-4o-') && modelId.match(/\d{4}-\d{2}-\d{2}/)) {
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		if (!modelId || typeof modelId !== 'string' || modelId.trim().length === 0) {
			throw new NodeOperationError(this.getNode(), 'Model ID is required and must be a non-empty string');
		}

		// Validate model mode
		if (!modelMode || !['fromList', 'byId'].includes(modelMode)) {
			throw new NodeOperationError(this.getNode(), 'Invalid model selection mode');
		}
		...
		try {
			...
			const errorDetails = getErrorDetails(error);
			throw new NodeOperationError(this.getNode(), `Model name validation failed: ${errorDetails.message}`);
		}

		const options = this.getNodeParameter('options', itemIndex, {}) as N8nNodeOptions;
		const usageMetadata = this.getNodeParameter('usageMetadata', itemIndex, {}) as UsageMetadata;
		...
		try {
			...
			const errorDetails = getErrorDetails(error);
			throw new NodeOperationError(this.getNode(), `Temperature validation failed: ${errorDetails.message}`);
		}

		try {
			validateNumericParameter(options.maxTokens, 'maxTokens', 1, 100000, true);
		} catch (error) {
			const errorDetails = getErrorDetails(error);
			throw new NodeOperationError(this.getNode(), `Max tokens validation failed: ${errorDetails.message}`);
		}
		...
		try {
			...
			const errorDetails = getErrorDetails(error);
			throw new NodeOperationError(this.getNode(), `Timeout validation failed: ${errorDetails.message}`);
		}
		...
		logger.debug('Configuration options: temperature=%s, maxTokens=%s, timeout=%s, hasUsageMetadata=%s',
			options.temperature, options.maxTokens, options.timeout, !!usageMetadata && Object.keys(usageMetadata).length > 0);
		
		const chatModel = new ReveniumTrackedChatOpenAI({
			apiKey: credentials.openaiApiKey, // Changed from openAIApiKey to apiKey for v0.6.3
			modelName,
		...
		}, { ...credentials, usageMetadata });

		logger.debug('Revenium Chat Model - returning LangChain ChatOpenAI with tracking');
